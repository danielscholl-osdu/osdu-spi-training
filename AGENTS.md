...
- The start page has no rail; the chapter rail belongs to lessons, where it shows position. Start leads with one promise: a headline that names the subject in one memorable line (“Understand the Azure SPI machinery.”) with its second line saying what the machinery is (the stack, the provider inside each service, the engineering that keeps them current), an intro that describes the machinery (a stack, a provider inside each service, fork workflows) and only then names SPI, a decorative machinery graphic beside it (generated; its prompt lives in docs/reference/start/), and one sentence defining CIMPL that links the name to the community project. Then “Introduction” with two alternatives, “Watch · 1 min” (the video opens in a modal over the page) and “Listen · 2 min” (the brief plays in place); no episode list there. Then the three meanings of SPI, then one lesson index grouped “The stack” (01, 02) and “Provider code and engineering” (03 to 07), 01 carrying the starting emphasis; a card shows its number, one title or question, and a description only when it adds information. No doors, no trace invitation, no second introduction block, and no second index: the round-trip figure and the six-place ladder live on the Field guides page. Reference below the index stays concise: the three meanings of SPI in the order stack, engineering system, the SPI inside each service, each card carrying its repository link and no lesson link; then one small Go deeper section at the end with the design guides, decision records, and the CIMPL reference, not collapsed. The masthead carries no Explore link until Explore is a distinct view; Audio deep dives and Field guides are two quiet icon links in the masthead and appear nowhere else on Start (not in the rail, not repeated on the page).
...
- The video opens in a modal from Start's Introduction: playback starts from that click, captions and the source-check notes stay inside the player, focus stays in the dialog, and closing (× or Escape) pauses the video, keeps the page position, and returns focus to Watch; closing starts nothing else. It pauses the dock and the dock pauses it. The video is not an episode and has no markers; keep it to the start page. The brief is the page's cue and plays in the dock; the full orientation is reachable from Resources.
