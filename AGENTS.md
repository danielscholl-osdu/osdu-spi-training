...
- “Try it” is the hands-on band: what to run or click to do what the lesson described, on the learner’s own personal GitHub account and Azure subscription. It is optional, collapsed to one line by default, expands in place without moving the page, opens no drawer, and stores nothing. Each band stands alone in any order and names its prerequisites, its cost (free on GitHub or billable on Azure), its rough time, the steps (each a command or click plus what to look for), the clean-up, and the runbook or CLI help it quotes at a named revision. Only the running example’s names (`opendes`, `dev1`) and placeholders such as `<name>` appear; no environment of ours does. The site still executes nothing and shows no live state. Features documented ahead of the code (`spi service refresh`, `--canonical-source`, the stale-sweep step) are out of bounds. A band never becomes required, scored, or tracked.
