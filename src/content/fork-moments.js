...
export const forkMoments = [
  {
    ...
    title: 'fork_upstream is printed again, not merged.',
  ...
  },
  {
    ...
    copy: 'Cascade Monitor dispatches the cascade once the sync PR merges. It merges main into fork_integration first, so the fix is there, then merges fork_upstream on top, then builds -P core,azure. This is the first time the provider compiles against the new shared code.',
    action: 'Merge main, then fork_upstream, into fork_integration',
    detail: 'cascade-run',
    time: 'After the sync PR merges',
    timeKind: 'Dispatched by the monitor, which runs every 6 hours',
  ...
  },
  {
    ...
    owner: 'Validation · on the PR and on main',
    title: 'Every eligible commit gets a digest, and a turn in dev1.',
    ...
    action: 'Push the candidate digest, then borrow a slot in dev1',
    detail: 'candidate',
...
export const meanwhile = [
  ...
  },
  {
    ...
    copy: 'Labels, rulesets, required checks.',
  },
];
