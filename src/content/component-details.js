export const componentDetails = {
  ...
  engineering: {
    ...
    body: 'Azure/osdu-spi is a template repository. Its own .github/workflows/ run only in the template; .github/template-workflows/ is what a fork receives into its .github/workflows/. Sync Template delivers changes daily at 08:00 UTC as a PR, and Settings Apply reconciles labels and rulesets on Mondays. A mirror fork turns template sync off because the service repository already carried it.',
    artifact: {
  ...
  running: {
    label: 'The pod in dev1',
    title: 'The environment the fork borrows was built by view 02.',
    body: 'dev1 is a stack like any other: spi up made it, Flux assembles it, and its deploy identity survives spi down. Several onboarded forks can share it, one service slot each, and a concurrency group serialises runs per service. While partition is pinned to a candidate, the other services keep running their canonical images, so a broken-but-ready candidate can fail a sibling’s suite.',
    artifact: {
  ...
  delivery: {
    ...
    body: 'The lock lives in osdu-flux with one key per service, PARTITION_IMAGE_DIGEST among them, and Flux substitutes the values at apply time. A fork run writes its digest with spi service pin --ephemeral, which also records the run id, the source commit, and the canonical image to restore, in the spi-stack.osdu.dev/pins annotation. The write is a compare-and-set: two runs cannot both own the partition slot.',
    artifact: {
      label: 'The pin',
      code: 'spi service pin partition --image ghcr.io/…@sha256:… \\\n  --ephemeral --run-id $GITHUB_RUN_ID --source-repo … --source-sha …',
    },
    source: 'imageLock',
  },
  proof: {
    ...
    body: 'The resolver binds each suite’s inputs from environment facts and three tokens the run mints: a privileged caller, a member, and one with no access. Each suite runs as docker run --env-file <suite>.env from the acceptance image under its own timeout. The verdict is the Surefire and Failsafe reports: exit zero, at least one test that was not skipped, no failures. A clean container exit alone proves nothing.',
    artifact: {
  ...
  'engineering-files': {
    ...
    title:
      'The workflows, the Dockerfile, and the descriptor are the fork’s, from the template.',
    body: '.github/ holds the workflows the template delivers; build/Dockerfile is the one canonical service image recipe; .spi/service.yaml declares the acceptance suites. None of them exist upstream. They arrive through Sync Template as pull requests and are protected from the upstream filter.',
    artifact: {
      label: 'Fork-owned, template-delivered',
      code: '.github/workflows/    from .github/template-workflows/\nbuild/Dockerfile      mcr.microsoft.com/openjdk/jdk:17-azurelinux\n.spi/service.yaml     schemaVersion: 3',
    },
    source: 'workflowSystem',
  },
  'fork-upstream': {
  ...
  'fork-integration': {
    ...
    title: 'The disposable workspace where the two trees meet.',
    body: 'Before each cascade the branch is reset to main. The cascade then merges fork_upstream in, resolves conflicts here if a person must, stamps versions, and builds -P core,azure. Its protection is relaxed on purpose so conflict resolution can push directly; it is cleaned up by Integration Branch Cleanup afterwards.',
    artifact: {
      label: 'Order of merges',
      code: '1. fork_integration ← main\n2. fork_integration ← fork_upstream\n3. mvn -P core,azure',
    },
    source: 'cascade',
  },
  'main-branch': {
    ...
    body: 'main receives the fork-owned fix through an ordinary pull request and the upstream change through the release/upstream-* PR the cascade opens. Both need CodeQL and Validation Summary to pass, and a person to approve. The rulesets that say so are reconciled every Monday by Settings Apply, so drift in the repository settings is corrected, not accumulated.',
    artifact: {
  ...
  'cascade-run': {
    ...
    body: 'The cascade is where the fork-owned fix, already on main, and the upstream change, on fork_upstream, compile together. It runs with pull_request_target so it can use the repository’s tokens, builds -P core,azure with Java 17, and opens a release/upstream-* PR labeled validated when the build and tests pass. Coverage is reported, not gated.',
    artifact: {
      label: 'Cascade build',
  ...
  'release-pr': {
    ...
    title: 'The cascade proposes; a person merges.',
    body: 'A clean cascade opens release/upstream-<version> against main with the label validated. Review is asymmetric on purpose: the sync PR into fork_upstream is generated and needs no reading, while the PR into main carries the combined tree and gets the same checks as any other change. Release Please keeps its own release PR alongside, bumping the version from the meta commit.',
    artifact: {
      label: 'Two PRs into main',
      code: 'release/upstream-<version>   label: validated\nrelease-please--branches--main',
    },
    source: 'cascade',
  },
  'release-tag': {
    ...
    body: 'When the release PR merges, Release Please tags main and publishes the release. The workflow adds <release-tag>-upstream-<upstream-version> so the fork release can always be traced to the upstream version it carries, then polls GHCR for the sha-* image validation pushed for that commit and adds the semantic-version tag to it. No new build runs.',
    artifact: {
  ...
  gate: {
    ...
    body: 'The gate runs before any Azure login. It refuses schedule and workflow_dispatch runs, PRs from other repositories, Dependabot, and fork_upstream, and it checks that the five onboarding values exist, .spi/service.yaml is present, and Docker Push succeeded. A refusal is a visible notice and the summary stays green, which is why green is not deployment evidence.',
    artifact: {
      label: 'Skip reasons, verbatim',
      code: 'pull request from another repository carries no deploy identity\nfork_upstream builds no Azure image\nrepository is not onboarded to a stack (missing: …)\nno .spi/service.yaml declares the suites\nno image was pushed',
    },
    source: 'validation',
  },
  trust: {
  ...
  descriptor: {
    ...
    body: '.spi/service.yaml names each suite, the image that runs it, its timeout, and the bindings it wants: a gateway URL, a partition, a token. The resolver in the run binds those from environment facts and minted tokens. This replaced sixteen pushed repository variables and a sticky boolean that used to say “validated”.',
    artifact: {
