...
| Codex              | OpenAI GPT-6 Astra (138 responses at medium effort, 572 at xhigh)                                  | Codex CLI and the ChatGPT Codex app | The five-file prototype, the source layout, and the first review response on Day 1, before the first Claude session; the lesson 01 rebuild; usability, editorial, and source-checked reviews on `codex/review-*` branches | 7 tasks, 710 responses: 15 review rounds, 3 implementation passes |
...
```mermaid
gantt
    title Keelson beads-work runs, Day 3 (CDT)
...
There is one more thing the record shows. The owner lost the thread twice late on the night of Day 3: "so you lost me. Are we good with our open tasks or there are still some left." and, a few minutes later, pasting the orchestrator's own status report back as a prompt. Both came after a night of dense status updates. A factory that reports too much is as hard to manage as one that reports too little.
...
What it was worth, in the owner's words on the afternoon of Day 3: "the keelson workflow beads-work is pretty powerful although takes time. It is important we scope the bead work being done worth the time effort. That is a learning." The value was in four places.
...
The design conversations in ChatGPT with GPT-6 Astra are where the site's shape came from. The story, map, inspector model ("Tell the story first. Let the map be inspected second."), the lesson grammar, the decision that the claim is the primary interaction and everything else is evidence, the reading-flow simplification of Start, and the illustrated lesson footer all arrived as pasted design feedback on Day 3 and Day 4. The owner's framing when the first round came in: "I still think we haven't landed on the exact right design." Eight rounds later, the design was settled enough to be written as rules.
...
NotebookLM produced every recording and the video, from sources the owner selected and later from a source the orchestrator wrote for it. It was fast and the owner valued the result: "the impact on the 2 deep dives ... are huge." It also had the project's most visible quality problem. The Codex editorial review called the recordings "where the AI voice is most noticeable," with the "massive" and "exactly" counts, and the training-experience review found six factual errors in the orientation episode with timestamps. The site's answer was to keep the recordings, correct them in marker notes, write replacement scripts for approval, and record the corrections as a rule: a recording is replaced from an approved script, never by editing its transcript. Recording the replacements is still open. The Gemini image model, suggested by the owner on the night of Day 3 ("I think you could even send something to gemini model api to create an image"), produced the hero strip and badges in the field-guide style with their prompts kept beside them; the footer badges on Day 4 were drawn by hand as SVG because the orchestrator could not call the model itself.

## What existed before the hackathon

Integrating a factory is not the same as building one. Before Day 1 there were: the three source repositories and their documentation, which the site explains and the content test resolves paths against; two supplied PDF guides, one of 103 pages; the beads CLI; Keelson with its beads rib and the beads-work workflow, both written earlier in the summer for other projects (the rib needed a fix during the event, and the workflow was installed on this project on the morning of Day 3); the Claude Code skills used for posters, diagrams, briefs, and pull request voice; the Chrome extension; the Codex CLI and its companion app; NotebookLM; and the five-file prototype Codex built on the afternoon of Day 1. Built during the four days: the site, the content standard in AGENTS.md, the content test, the tracker artifact, the bead conventions, and the delegation rules.

## One change, end to end

The Go deeper shelf (bead fn-m7u, Day 4) is a complete trace through the factory.
...
The record does not measure the owner's time. An estimate from message timestamps, bridging gaps of under thirty minutes, gives about 15 hours of active attention across the four days, and about 21 with gaps under an hour bridged: two to three hours on Day 1, four to seven on Day 2, eight to ten on Day 3, and two on the morning of Day 4. That excludes time in Codex, ChatGPT, and NotebookLM, which the Claude record does not see, and it counts a paste as a moment rather than the reading that preceded it. "One person" is true; "little effort" is not established.

## Where the tokens went

Three of the channels keep a usage ledger the owner can read: Claude Code, Keelson, and Codex. Between the first Codex task on Day 1 and the Pages run for the final build commit on Day 4 they recorded 30.8 million new input tokens, 904 million cached input tokens, and 4.05 million output tokens. ChatGPT, NotebookLM, and the Gemini image model are not in these figures; they keep no ledger the owner can export. The [usage analysis](usage/README.md) has the per-model tables and the accounting rules, and the chart sources sit beside it.

![Where the tokens went, by channel](usage/usage-by-channel.png)

Three things stand out, and each bears on the division of labour described above.

The orchestrator wrote the most. Claude Code's main sessions produced 2.03 million output tokens, more than the Keelson implementers (0.65 million) and Codex (0.43 million) together. This is the measurable form of a point made earlier: the orchestrator was not only coordinating. It wrote the lesson content, the briefs, the tracker revisions, the bead descriptions, and the documentation, and it delegated the code.

Keelson consumed the most new input. The implement node alone took 6.3 million new input tokens across its 48 node turns, because every run starts from a fresh context, reads the repository, and carries the captured diff through the review lanes; the three lanes and the re-review took another 3.5 million. That is the price of the isolation that made the runs trustworthy. A run inherits nothing from the orchestrator, so each one pays to read the codebase again.

![Who inside each channel used the tokens](usage/usage-by-role.png)

Almost all input was cached. Across the three channels 96.7% of input tokens were cache reads. A 1,301-turn orchestrator session re-reads its whole context on every turn, and a Keelson run carries its diff through every node; without caching, neither would have been affordable. The cached column dwarfs everything else on a linear scale, which is why the charts keep it apart.

![Where the tokens went, by day](usage/usage-by-day.png)

By day the shape follows the narrative. Day 1 and Day 2, before the workbench, are Claude Code and Codex alone, at 3.2 and 4.6 million new input plus output. Day 3, when the tracker and the workbench arrived and thirteen runs went through, carried 24.4 million, seventy percent of the four-day total, with Keelson the largest single channel. Day 4 fell back to 2.6 million for polish and closure. Model choice tracked the same turn: Opus 5 for Days 1 and 2 and Fable 5.1 from Day 3 on the Claude side; inside Keelson, GPT-6 Astra planned and triaged, GPT-5.6 Sol implemented, and GPT-5.6 Terra reviewed; Codex ran entirely on GPT-6 Astra, mostly at its highest effort setting.

These are provider-reported counts. They are not a bill, and they are not a measure of equivalent work across vendors. The Claude Code cost ledger in the appendix is the CLI's own running estimate; Keelson's usage is metered by the Copilot subscription, and Codex by its own.

## Recovering after a session ended

The orchestrator session was compacted eight times, and each resume started from the same three things: the tracker's memories and ready queue (`bd prime` prints the four memories and the queue), the tracker artifact's state line naming the commit on main, and the content standard. The delegation rule the owner stated on the afternoon of Day 3 was written into memory at 14:48 and applied on Day 4 at 11:05, six compactions later, when a run was cancelled in favour of a subagent without the rule being restated. A Keelson run is durable on its own: its plan, node outputs, and report stay in the workbench database, and the case study you are reading recovered run timings, model routing, review candidates, and approval text from that ledger a day later.
...
- **Media churn.** Four orientation recordings in three hours on Day 2, then both introductory recordings taken off the deep-dives listing on the evening of Day 3. The lesson written into the standard afterwards, script first and record from the approved script, would have saved most of the afternoon of Day 2.
...
- [The video overview](media/the-one-man-ai-software-factory.mp4) (1 min) and [the audio deep dive](media/the-four-day-ai-software-factory.m4a) (45 min), both generated by NotebookLM from this record; the deployed copies play in a browser from the [short case study](README.md).
