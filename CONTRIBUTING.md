...
Structured lessons put compact claim controls inside the map workspace, after its title row and before the selected sentence, reason, and map. Selecting a claim updates the explanation and map emphasis in place; selecting it again is idempotent. Only the explicit **How we know** link opens evidence. Structured lessons add zero-based selection qualifiers when intent would otherwise be ambiguous: claim evidence uses `?detail=azureimpl&claim=0`, while a trace uses `?detail=azureimpl&hop=3`. Unqualified legacy URLs remain valid; when a component matches both, claim evidence wins, otherwise a matching hop restores the trace.

After the optional Try it band, every map lesson uses one Go deeper shelf. Render only applicable rows, in this order: Follow the example, Hear it explained, Field guides, Sources. Rows start closed; each shows a badge from `src/components/badges.js`, a bold title, and a short preview derived from counts in the existing content (hops, excerpts and minutes, diagrams and posters, references). Titles and links appear only when a row is opened. Audio cue chips and `[data-listen-now]` sit inside the opened Hear it explained row. The Try it band is a banner with its own badge (`badge`), the summary's title and qualifier, and a one-sentence `outcome`. The shelf does not depend on the claim grammar.

A lesson whose subject is a lifecycle sets `lesson: 'lifecycle'` on its chapter; today only `bring-up` does. Its stage bar is the main workspace control: a stage link updates the map, the stage explanation, its **Look closer** evidence entry, and the easy mistake together, and leaves that explanation readable just below the sticky stage bar. The one bottom control is labeled `Continue to <next stage> →`, brings the next stage's explanation into view, and moves focus to its heading; the last stage has none. Its claims render as statements (headline and reason) with no selection state after the stage-driven workspace and before the easy mistake, still feed **What you can now say**, and keep their published `?claim=` evidence routes resolving. There is no Lesson focus toggle; nothing recedes unless a running-example hop is traced. An easy mistake whose collection route points at another lesson can name an in-lesson target with `here: { '<chapter>': '<route>' }` in `myths.js`, or a native field guide rendered beside that lesson with `guideHere: { '<chapter>': '<guide id>' }`, which opens the guide in place.

A lesson whose subject is one run in order sets `lesson: 'steps'`; today only `handshake` does. Its running-example hops render in the claim-control slot as buttons (`data-step`), each with one **How we know** link. A step selection is `#handshake?hop=<n>`; its evidence is `#handshake?detail=<id>&hop=<n>`; a bare `?detail=` that names a step's component still selects that step, so published links keep their view. Selecting a step moves the seam's lock-and-pod state, focuses the step's component, and shows the hop copy in place without opening the drawer; a component click keeps the step. Its claims render as statements after the workspace (`#chapter-statements`), and its example row is not rendered.

Every map lesson with a running example, other than a step lesson, renders it as a collapsed native disclosure after the lesson outcomes, exit, and optional Try it band. The disclosure is the Follow the example row of the Go deeper shelf on those map lessons. A bare lesson route leaves it closed; a matching hop route opens it and restores the trace. Expanding or collapsing the disclosure alone changes no route or persistent state.

Each component detail contains `label`, `title`, `body`, `artifact: { label, code }`, and a `source` key from `src/content/sources.js`. `resolveDetail` overlays an optional `here[chapterKey]` record without changing the base detail. `label` remains metadata and is the fallback for drawer context; it never implies ownership. Add `owner` only when the source or runtime owner is explicit. The compact metadata reads `<context> · <owner>`, with the separator inside the optional owner wrapper so an absent owner leaves context alone.
...
- `prerequisites` is a nonempty array of chips `{ kind, label, text?, shell?, chapter?, sources }`. `kind` is `tool`, `lesson`, `access`, or `browser`; `label` is at most 48 characters; `text` is an optional short clause under it; `shell` (`posix` or `powershell`) shows the chip only for that shell; `chapter` (lesson chips only) links the label to that lesson; each `sources` list contains keys from `src/content/sources.js` so shared setup is linked instead of repeated.
- `footprint` states the three places in order, `workstation`, `github`, `azure`, each `{ place, state, note }` with `state` one of `untouched`, `reads`, `changes`, or `bills` and a short `note`. Only a variant whose access is `Azure resources billed separately` may, and must, mark Azure as `bills`.
- `time` has nonempty `active`, `wait`, and `cleanup` descriptions and `minutes: { active, wait, cleanup }` as whole planning minutes for the effort bar (`active` above zero; zero omits a segment). State when waiting or cleanup does not apply; do not derive estimates from timeouts, and keep the qualified wording in the descriptions, which the bar's legend shows.
- `steps` is a nonempty ordered array. Each step has exactly one nonempty `command` or `click`, a `touch` of `{ kind, note }` saying what the step does (`reads`, `local`, `creates`, or `removes`, with a clause of at most 60 characters and no final period) that renders before the action, plus `expect` and optional source keys. The renderer shows `effects` and the footprint before these steps and never executes their actions.
- `alternate` has a nonempty `observation` and `next`. `cleanup` has nonempty `steps` using the same action-and-touch-and-observation shape, an optional `ledger` of `{ columns, rows }` where each row is `{ item, after }` and `after` states `kept` or `removed` per column, and a nonempty `remains` statement.
- `sources` is a nonempty list of source keys for the quoted runbook, CLI help, or source artifact.
- `tested` has nonempty `cli`, `stack`, `template`, `shell`, `os`, and ISO `date` values. For a field that does not apply, write an explicit reason instead of inventing a version. Keep these records for publication review; do not render walkthrough dates or status in the learner interface.
...
Keep the overview and lifecycle on the same map renderer. When changing interaction, check one visible selected element, its matching explanation at the map, browser back/forward, reload, and a phone-sized viewport. Claim selection changes emphasis without opening evidence or moving the page; only **How we know**, a component, a hop, an explicit map jump, or a detail route opens the evidence drawer. One local helper in `main.js` opens every containing native disclosure before guide-route, audio-marker, or local-myth target scrolling. `scrollMemory` stays transient and keys entries by the full hash; it records only the open structured shelf-row IDs and reopens them before restoring the saved offset on Back/Forward. It does not persist Try it or nested guide state, and fresh navigation starts closed. The drawer starts closed and overlays the full-width desktop map. Below 760px it opens as a bottom sheet. Its × button and Escape close it and return focus to the opener, leaving the selected component and any map state it drives in place. There is no Lesson focus or Explore map toggle; receding and the hover or focus Inspect affordance are the only behavior.
...
Review reference material is preserved separately in `docs/process/reviews/concept-review.html`. The review HTML is excluded from formatting. See `docs/process/iterations/` for the iteration records and `docs/audio-source-notes.md` before producing narration.
