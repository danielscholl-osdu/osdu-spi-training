...
Multiple variants are complete alternatives, such as a future lesson 02 route without Azure and one with Azure. The collapsed summary names each variant's access and active effort. Commands use only the running example's partition name (`opendes`) and marked placeholders such as `<name>` for the environment; the site names no environment, because each learner creates their own with `--name`. Keep prerequisites sufficient to start the activity, state resource effects before steps, and identify what remains after cleanup. The site must not promise that an activity is free, execute a command, imply live state, or track completion.
