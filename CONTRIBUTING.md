...
After the unchanged Try it band, structured map lessons use one Go deeper shelf. Render only applicable rows, in this order: Follow the example, Hear it explained, Field guides, Sources. Rows start closed; previews are derived from the existing example, guide, poster, and source titles. Audio cue chips are always-visible actions beside the Hear it explained disclosure, not controls inside its `summary`; `[data-listen-now]` stays in the same row wrapper. The current structured boundary is lessons 01 to 03. Lessons 04 to 06 keep their legacy optional layout.
